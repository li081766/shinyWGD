...
function jointTreePlot(InputData) {
    ...
    // addScript("https://d3js.org/d3.v3.min.js");
    jointTreeBuilding("#jointtree_plot", ksTreeJson, ksPeakInfo, timeTreeJson, wgdTableInfo, height, width);
}
...
function speciesTreeUpdatedPlot(InputData) {
    ...
    var treeSvg = d3.select("#speciesTree_plot")
        .select("svg")
...
function jointTreeBuilding(selector, ksTreeJson, ksPeakInfo, timeTreeJson, wgdTableInfo, height, width) {
    var svgFile;
    //load the d3 v3
    var script = document.createElement('script');
    script.src = 'https://d3js.org/d3.v3.min.js';
    ...
    script.onload = function () {
        ...
        if (typeof ksTreeJson !== 'undefined' && typeof timeTreeJson !== 'undefined') {
            // Add the timeTree to right part of the plot
        ...
        downloadSVG("jointTreePlotDownload", "jointtree_plot", svgFile);
    }
}
...
function buildKsTree(selector, vis, ksTreeJson, ksPeakInfo, w, h, ori) {
    ...
    /* vis.selectAll('.ks-rule-line')
        .data(yscale.ticks(11))
        .enter().append('svg:line')
        .attr('class', 'ks-rule-line')
        .attr('y1', 0)
        .attr('y2', h)
        .attr('x1', yscale)
        .attr('x2', yscale)
...
function drawGeologicTimeBar(maxLength, vis, yscale, h, ori) {
    ...
    var drawTimeData = geologicData
        .filter(function (d) {
            if (ori === "right") {
                return d.end < maxLength * 100;
            } else {
                return d.start < maxLength * 100;
            }
        });

    // console.log("drawTimeData", drawTimeData);
    ...
    vis.selectAll('.time_rect')
        ...
        .attr("data-tippy-content", (d) => {
            if (ori === "right") {
                return "<font color='#4DFFFF'>" + d.name + "</font>: <font color='orange'>" + d.end
                    + "</font> <b>&#8656</b> <font color='#00DB00'>"
                    + d.start + "</font>";
            } else {
                return "<font color='#4DFFFF'>" + d.name + "</font>: <font color='#00DB00'>" + d.start
            ...
            .on('click', function (d) {
                if (leafPopUpMenu.style('visibility') == 'visible') {
                    ...
                    /* vis.selectAll('g.leaf.node')
                        .on('click', function () {
                            var clickedLeafNode = this;
                            console.log('old', clickedLeafNode);

                            function addSymbol(textElement) {
                                // define the symbol menu options
                                var symbols = [
                                    { name: 'Circle', type: 'circle' },
                                    { name: 'Cross', type: 'cross' },
                                    { name: 'Diamond', type: 'diamond' },
                                ...
                                var symbolMenu = d3.select('body')
                                    .append('div')
                                    .attr('class', 'symbol-menu')
                                    .style('position', 'absolute')
                                    .style('top', (d3.event.pageY + 10) + 'px')
                                    .style('left', (d3.event.pageX + 10) + 'px')
                                    .style('padding', '10px')
                                ...
                                symbolMenu.append('p').text('Select a symbol:');

                                // add the symbol options to the menu
                                var symbolOptions = symbolMenu.selectAll('div')
                                    .data(symbols)
                                    .enter()
                                    .append('div')
                                    .text(function (d) {
                                    ...
                                    .on('click', function (d) {
                                        var symbol = d3.svg.symbol()
                                            .type(d.type)
                                        var textColor = d3.select(clickedLeafNode).select('text').style('fill');
                                        d3.select(clickedLeafNode)
                                            .append('path')
                                            .attr('d', symbol)
                                            .attr('stroke', textColor)
                                            .attr('fill', textColor)
                            ...
                            d3.select('#add-symbol').on('click', function () {
                                addSymbol(textElement);
                            });
                            // add your code to handle the click event on the clickedLeafNode here
                        }); */

                    // Attach event listener to trigger the popup menu
                    /*                     showPopupMenu();
                    
                                        function showPopupMenu() {
                                            // Calculate the desired position relative to the mouse event
                                            var desiredLeft = d3.event.pageX + 10; // Set the desired left position with an offset
                                            var desiredTop = d3.event.pageY - 10; // Set the desired top position with an offset
                    
                                            // Set the position and visibility of the popup menu
                                            popUpMenu.style('left', desiredLeft + 'px')
