...
function drawGeologicTimeBarV2(maxLength, vis, yscale, h, ori) {
    var geologicData = [
        { name: 'Quaternary', start: 0, end: 2.6 },
        { name: 'Neogene', start: 2.6, end: 23 },
        { name: 'Paleogene', start: 23, end: 66 },
    ...
    var drawTimeData = geologicData
        .filter(function (d) {
            if (ori === "right") {
                return d.end < maxLength;
            } else {
                return d.start < maxLength;
            }
        });
        ...
        .on('click', function (d) {
            if (popUpMenu.style('visibility') == 'visible') {
                ...
                var timePeriodRect = d3.selectAll('.add-btn').on('click', function () {
                    var timeInputStart = document.getElementById('time-input-start').value;
                    var timeInputEnd = document.getElementById('time-input-end').value;
                    var x1 = yscale(timeInputStart);
                    var x2 = yscale(timeInputEnd);
                    if (ori === "right") {
                        var rectWidth = (x1 - x2);
                    } else {
                        var rectWidth = (x2 - x1);
    ...
    vis.selectAll('.bar-label')
        ...
        .attr('class', 'bar_label')
        .attr('x', function (d) {
            var barWidth = yscale(d.end) - yscale(d.start);
            var translateX = yscale(d.start) + barWidth / 2;
            if (d.start == 0) {
                if (ori === 'right') {
                    translateX += 30;
                } else {
