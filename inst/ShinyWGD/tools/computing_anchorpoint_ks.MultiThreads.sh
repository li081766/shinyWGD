...
if [ $# -lt 3 ]; then
    echo "Usage: $0 <anchorpoints_file> <fasta_file> <thread_num>"
    exit 1
fi
...
perform_alignment() {
    ...
    grep -A 1 "$gene_x" "$fasta_file" | tail -n 1 >> "$tmp_fasta"
    echo ">${gene_y}" >> "$tmp_fasta"
    grep -A 1 "$gene_y" "$fasta_file" | tail -n 1 >> "$tmp_fasta"

    # Perform alignment using prank (assuming codon alignment)
    aligned_file="${gene_x}_${gene_y}_nuc.aligned"
	module load gcc/x86_64/6.3; /home/jiali/Software/Prank/170703/development/prank/bin/prank -codon -F -once -d="$tmp_fasta" -o="$aligned_file"
